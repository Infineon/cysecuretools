...
import struct
import getpass
import cysecuretools.imgtool.keys as keys
import sys
from cysecuretools.imgtool import image, imgtool_version
from cysecuretools.imgtool.version import decode_version
from .keys import (
...
@click.option('--custom-tlv', 'custom_tlvs', required=False, nargs=3,
              default=[], multiple=True, metavar='[tag] [length] [value]',
...
@click.option('--boot-record', metavar='sw_type', help='Create CBOR encoded '
              ...
              '[max. 12 characters]', default="TestRecord")
...
def sign(key, public_key_format, align, version, pad_sig, header_size,
         ...
         erased_val, save_enctlv, security_counter, custom_tlvs, boot_record):
    ...
    for tlv in custom_tlvs:
        tag = int(tlv[0], 0)
        if tag in custom_tlv:
            raise click.UsageError('Custom TLV %s already exists.' % hex(tag))
        elif tag in image.TLV_VALUES.values():
            raise click.UsageError('Custom TLV %s conflicts with predefined TLV.' % hex(tag))
        else:
            length = tlv[1]
            value = tlv[2]
            if 's' in length:
                custom_tlv[tag] = struct.pack(length, bytes.fromhex(value))
            else:
                custom_tlv[tag] = struct.pack(length, int(value))

    img.create(key, public_key_format, enckey, dependencies, boot_record, custom_tlv)
